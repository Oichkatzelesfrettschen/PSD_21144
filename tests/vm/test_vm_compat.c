...
/* Basic stand-in structures for testing the compatibility layer. */
...
struct vm_pseudo_segment {
	struct vmspace*	 ps_vmspace;  /* owning vmspace */
	struct vm_data*	 ps_data;	  /* data segment */
	struct vm_stack* ps_stack;	  /* stack segment */
	struct vm_text*	 ps_text;	  /* text segment */
	void*			 ps_minsaddr; /* min stack address */
	void*			 ps_maxsaddr; /* max stack address */
};

struct vmspace {
	struct vm_pseudo_segment vm_psegment; /* pseudo-segment description */
};

struct proc {
	struct vmspace* p_vmspace; /* associated vmspace */
};
